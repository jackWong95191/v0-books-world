...
export default async function DiscoverPage() {
  ...
  // Fetch profile data for bookstore owners
  ...
  const { data: bookstoreProfiles } = bookstoreUserIds.length
    ? await supabase.from("profiles").select("id, username, full_name, avatar_url").in("id", bookstoreUserIds)
    : { data: [] }

  // Combine bookstores with their owner profiles
  ...
  // Fetch profile data for book owners
  const bookOwnerIds = booksData?.map((b) => b.owner_id).filter(Boolean) || []
  const { data: bookOwnerProfiles } = bookOwnerIds.length
    ? await supabase.from("profiles").select("id, username, full_name, avatar_url").in("id", bookOwnerIds)
    : { data: [] }
